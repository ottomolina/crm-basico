...
export const FormCliente = () => {
    ...
    const estados = getCatalogo().listaEstado.filter((e:any)=>`${e.tabla}`.includes('CRM_CLIENTE'))
    ...
    useEffect(() => {
        if(clienteid) {
            getClientes(+clienteid).then((result) => {
                if(result.length === 0) {
                    alert.error(new ErrorHttp(1, 'No se encontró el cliente.'));
                } else {
    ...
    const handleGuardar = (event:any) => {
        ...
        if(clienteid) {
            ...
            promise = updateCliente(cliente);
        } else {
            promise = addCliente(cliente);
        }
        promise.then(resp => {
            alert.dismissLoader();
